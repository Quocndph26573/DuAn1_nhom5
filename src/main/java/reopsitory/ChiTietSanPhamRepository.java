...
public class ChiTietSanPhamRepository {
    ...
    public void update(Integer id, ChiTietSanPham ctsp){
        session.beginTransaction();
        String query = "update ChiTietSanPham set Ma =:ma,Ten =: ten, IdLoaiSP =:IdLoaiSP, "
                + "IdThuongHieu =:IdThuongHieu,IdChatLieu =:IdChatLieu, IdSize =:IdSize, ngaySua =:ngaySua, Gia =: gia,soLuong=:soLuong where id =:id";
        Query q = session.createQuery(query);
        q.setParameter("ma", ctsp.getMa());
        q.setParameter("ten", ctsp.getTen());
        q.setParameter("IdLoaiSP", ctsp.getIdLoaiSP());
        q.setParameter("IdThuongHieu", ctsp.getIdThuongHieu());
        q.setParameter("IdChatLieu", ctsp.getIdChatLieu());
    ...
    public ArrayList<ChiTietSanPham> loc(ChiTietSanPham ctsp){
        session = HibernateConfig.getFACTORY().openSession();
        Query q = session.createQuery("from ChiTietSanPham where IdLoaiSP =:IdLoaiSP and IdThuongHieu =:IdThuongHieu and IdChatLieu =:IdChatLieu and IdSize =:IdSize"
